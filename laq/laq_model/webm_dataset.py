...
class WebmVideoDataset(Dataset):
    ...
    Args:
        ...
        frame_offset: Number of frames between the pair (default: 5)
    ...
    def __init__(
        ...
        # Build index: list of (video_path, start_frame_idx)
        self.samples = self._build_sample_index()
        print(f"Total samples: {len(self.samples)}")
    
    def _build_sample_index(self) -> List[Tuple[str, int]]:
        """Build list of (video_path, start_frame) tuples."""
        samples = []
        ...
        for video_path in self.video_files:
            ...
            if frame_count <= self.frame_offset:
                continue
            
            # Valid start frames (need room for frame_offset)
            max_start = frame_count - self.frame_offset - 1
            ...
            if self.num_samples_per_video is None:
                ...
                # Sample uniformly
                if max_start > 0:
                    step = max(1, max_start // self.num_samples_per_video)
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Read second frame
        cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame + self.frame_offset)
        ret2, frame2 = cap.read()
        
        cap.release()
        
        if not ret1 or not ret2:
            # Fallback: return random noise (shouldn't happen often)
            print(f"Warning: Failed to read frames from {video_path}")
            dummy = torch.randn(3, 2, *self.frame_size)
            return {"video": dummy, "video_id": "error"}
